    ...
    public void run() {
        ...
        while (true)
        {
            ...
            try
            {
                ...
                Snake snakeClietn = convertJsonValueToSnake(received);
                Snake snake = null;
                ...
                // ar is the vector storing client of active userss
                ...
                if(isloggedin){
                   ...
                   snake = generateSanke(index);
                   world = generateWorldSpace();
                    myMap.put("snake",snake);
                    myMap.put("world",world);

                    String returnString = gson.toJson(myMap);
                    dos.writeUTF(returnString);
               }
               else{
                    Dot  head = snakeClietn.dots.get(0);
                    if(isConllisionBetweenSnakeHeadAndWorldSpace(head) && totalItems != 0){
                        int x = (int)Math.round((double)head.x/32);
                        int y = (int)Math.round((double)head.y/32);

                        world[x][y] = SpriteType.BRICK_WALL;
                        totalItems --;
                        myMap.put("snake",snake);
                        myMap.put("world",world);
                    ...
                    if(totalItems == 0){
                        totalItems = WorlSize.TOTAL_FOODS;
                        world = generateWorldSpace();
                        myMap.put("snake",snake);
                        myMap.put("world",world);
